...
class Solution:
    ...
    def solve(self, first, second):
        ...
        return (
            ...
            and self.solve(first.left, second.left)
            and self.solve(first.right, second.right)
        )


def main():
    test.validate(True, Solution().solve(tree.Tree("a"), tree.Tree("a")))
    test.validate(False, Solution().solve(tree.Tree("b"), tree.Tree("a")))
    test.validate(
        True,
        Solution().solve(
            tree.Tree(
